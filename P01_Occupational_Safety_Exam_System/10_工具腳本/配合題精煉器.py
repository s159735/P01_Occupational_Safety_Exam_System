...
# ============================================================
# 📚 配合題精煉器 v10.6.0
# 職責：補齊配合題 leftItems / rightItems（含干擾選項）
# 格式：國家考試標準格式（選項區在上、作答區在下）
# 更新日期：2026-07-21
# ============================================================
...
CONFIG = {
    ...
    "OUTPUT_SUFFIX": "_精煉",
    "DISTRACTORS_COUNT": 3,      # 干擾選項數量（2~3）
}

# ============================================================
# 📋 手動對照表（請依序填入每一題的 leftItems + 干擾選項）
# ============================================================
...
class MatchRefiner:
    ...
    def get_question_data(self, q):
        """從對照表取得題目資料，若無則使用備用標籤"""
        q_id = q.get("id", "")
        answer = q.get("answer", [])
    ...
    def refine_question(self, q):
        ...
        # 取得題目資料
        data = self.get_question_data(q)
        left_items = data["leftItems"]
        right_items = data["rightItems"]
        distractors = data["distractors"]
        
        # ============================================================
        # 🎯 核心：生成 rightItems（含干擾選項 + 隨機排列）
        # ============================================================
        
        # 1. 複製正確答案作為 rightItems 基礎
        final_right_items = answer[:]
        ...
        if distractors:
            ...
            # 若無指定干擾選項，從備用干擾詞庫中隨機選取
            fallback_distractors = ["干擾選項A", "干擾選項B", "干擾選項C"]
            random.shuffle(fallback_distractors)
            selected_distractors = fallback_distractors[:self.distractors_count]
        
        # 3. 只取需要的數量（2~3 個）
        selected_distractors = selected_distractors[:self.distractors_count]
        
        # 4. 加入 rightItems
        final_right_items.extend(selected_distractors)
        ...
        q["rightItems"] = final_right_items  # ✅ 包含干擾選項且已隨機排列
    ...
    def run(self):
        print("\n" + "=" * 70)
        print("📚 配合題精煉器 v10.6.0")
        print("   職責：補齊配合題 leftItems / rightItems（含干擾選項）")
        print("   格式：國家考試標準格式（選項區在上、作答區在下）")
        print("=" * 70)
        ...
        if self.output_mode == "new":
            ...
            output_path = self.input_file.parent / f"{stem}{self.output_suffix}.json"
        else:
