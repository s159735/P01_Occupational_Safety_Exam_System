# ============================================================
# 🚀 FastAPI 後端入口 - 乙級職安衛模擬測驗系統 v10.2.1
...
#   5. 🆕 術科強制計算題 2 題（calc: 2~2）
#   6. 🆕 修復術科總數超過 10 題的問題（截斷至 10 題）
#   7. 🆕 修復配合題超過 2 題的問題（補充邏輯限制配額）
# ============================================================
...
def load_questions_from_json(mode: str):
    ...
    if mode == "technical":
        print("   ✅ 術科模式：計算2 + 配合1~2 + 填空1~2 + 排序1~2 + 複選1~2 + 是非1 + 連連看1")
    ...
    if mode == "technical":
        # 分離各題型
        calc_questions = [q for q in result if q.get('type') == 'calc']
        match_questions = [q for q in result if q.get('type') == 'match']
        fill_questions = [q for q in result if q.get('type') == 'fill']
        sort_questions = [q for q in result if q.get('type') == 'sequencing']
        multiple_questions = [q for q in result if q.get('type') == 'multiple']
        truefalse_questions = [q for q in result if q.get('type') == 'truefalse']
        link_questions = [q for q in result if q.get('type') == 'link']
        
        print(f"   📊 題庫統計:")
        print(f"      計算: {len(calc_questions)} 題")
        print(f"      配合: {len(match_questions)} 題")
        print(f"      填空: {len(fill_questions)} 題")
        print(f"      排序: {len(sort_questions)} 題")
        print(f"      複選: {len(multiple_questions)} 題")
        print(f"      是非: {len(truefalse_questions)} 題")
        print(f"      連連看: {len(link_questions)} 題")
        
        # ---- 固定數量 ----
        # 計算題：2 題（強制）
        if len(calc_questions) >= 2:
            selected_calc = random.sample(calc_questions, 2)
        else:
            selected_calc = calc_questions
            print(f"   ⚠️ 計算題不足 2 題，僅有 {len(calc_questions)} 題")
        
        # 是非題：1 題（強制）
        if len(truefalse_questions) >= 1:
            selected_truefalse = random.sample(truefalse_questions, 1)
        else:
            selected_truefalse = truefalse_questions
            print(f"   ⚠️ 是非題不足 1 題，僅有 {len(truefalse_questions)} 題")
        
        # 連連看：1 題（強制）
        if len(link_questions) >= 1:
            selected_link = random.sample(link_questions, 1)
        else:
            selected_link = link_questions
            print(f"   ⚠️ 連連看不足 1 題，僅有 {len(link_questions)} 題")
        
        # ---- 隨機數量（1~2 題） ----
        def select_questions(q_list, min_count=1, max_count=2, type_name=""):
            if not q_list:
                print(f"   ⚠️ {type_name} 題庫為空")
                return []
            count = random.randint(min_count, min(max_count, len(q_list)))
            return random.sample(q_list, count)
        
        selected_match = select_questions(match_questions, 1, 2, "配合題")
        selected_fill = select_questions(fill_questions, 1, 2, "填空題")
        selected_sort = select_questions(sort_questions, 1, 2, "排序題")
        selected_multiple = select_questions(multiple_questions, 1, 2, "複選題")
        
        # ---- 合併所有選取的題目 ----
        selected = []
        selected.extend(selected_calc)
        selected.extend(selected_match)
        selected.extend(selected_fill)
        selected.extend(selected_sort)
        selected.extend(selected_multiple)
        selected.extend(selected_truefalse)
        selected.extend(selected_link)
        ...
        # ✅ 如果超過 10 題，隨機刪減到 10 題
        if len(selected) > 10:
            print(f"   ⚠️ 選取了 {len(selected)} 題，隨機刪減至 10 題")
            selected = random.sample(selected, 10)
        
        # ✅ 確保總數為 10 題（如果不足，從「未滿配額的題型」中補充）
        while len(selected) < 10:
            ...
            # 找出還有配額空間的題型
            available_types = []
            for t, limit in quota_limits.items():
                if current_counts.get(t, 0) < limit:
                    available_types.append(t)
            
            # 從 result 中找出尚未被選取且屬於可用題型的題目
            selected_ids = [q.get('id') for q in selected]
            remaining = []
            for q in result:
                if q.get('id') not in selected_ids and q.get('type') in available_types:
                    remaining.append(q)
            
            if remaining:
                extra = random.sample(remaining, min(10 - len(selected), len(remaining)))
                selected.extend(extra)
                extra_types = [q.get('type') for q in extra]
                print(f"   ⚠️ 補充 {len(extra)} 題至總數 10 題（從 {extra_types} 補充）")
            else:
                # 如果沒有可用題型，從所有剩餘題目補充（最後手段）
                remaining_all = [q for q in result if q.get('id') not in selected_ids]
                if remaining_all:
                    extra = random.sample(remaining_all, min(10 - len(selected), len(remaining_all)))
                    selected.extend(extra)
                    print(f"   ⚠️ 緊急補充 {len(extra)} 題至總數 10 題")
                else:
                    break
        ...
        for t, count in final_counts.items():
            print(f"      {t}: {count} 題")
        print(f"      總計: {len(selected)} 題")
    ...
    print(f"   ✅ 最終選取 {len(selected)} 題 ({mode_label})")
...
if __name__ == "__main__":
    ...
    print("   - 術科: 計算2 + 配合1~2 + 填空1~2 + 排序1~2 + 複選1~2 + 是非1 + 連連看1 = 固定10題")
    print("   - 計算: 計算20 題")
    print("   ✅ 術科強制計算題 2 題")
    print("=" * 60)
