...
(function() {
    ...
    function renderChoice(q, index, container) {
        ...
        if (typeBadge) {
            var typeLabel = q.typeLabel || '選擇題(單選題)';
        ...
        helper.style.cssText = 'margin-top:8px;padding:6px 12px;font-size:14px;color:#999;text-align:left;';
        helper.textContent = '點選 ○ 圓形選鈕進行作答';
    ...
    function renderMultipleChoice(q, index, container) {
        ...
        var typeBadge = document.getElementById('questionTypeBadge');
        if (typeBadge) {
            var typeLabel = q.typeLabel || '選擇題(複選題)';
            typeBadge.textContent = typeLabel + ' (' + totalPoints + ')';
        }
        ...
        var helper = document.createElement('div');
        helper.style.cssText = 'margin-top:8px;padding:6px 12px;font-size:14px;color:#999;text-align:left;';
        helper.textContent = '點選 ☑ 方形勾選框進行作答 (可複選)';
        container.appendChild(helper);
